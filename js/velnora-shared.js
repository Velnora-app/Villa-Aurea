...
/** Numéro WhatsApp de la conciergerie (Stéphane), villa Aurea — voir villa-config.json → property.conciergeWhatsApp. */
const VLN_CONCIERGE_WA = '33647821590';

const VLN_HISTORY_KEY = 'velnoraChatHistory';
const VLN_HISTORY_MAX = 40; // borne la taille du JSON en localStorage sur un long séjour

/**
 * Restaure l'historique du chat assistant depuis l'appareil (persistant
 * entre écrans et sessions). Repli silencieux sur un historique vide si
 * l'entrée est absente, corrompue, ou si le stockage est indisponible.
 */
function vlnLoadHistory(){
  try{
    const raw = localStorage.getItem(VLN_HISTORY_KEY);
...
/**
 * Persiste l'historique du chat, tronqué aux VLN_HISTORY_MAX derniers
 * échanges. Repli silencieux si le stockage est indisponible (Safari
 * navigation privée, quota dépassé…) — l'assistant reste utilisable,
 * seule la persistance est perdue.
 */
function vlnSaveHistory(history){
  try{
    const trimmed = history.length > VLN_HISTORY_MAX ? history.slice(-VLN_HISTORY_MAX) : history;
    localStorage.setItem(VLN_HISTORY_KEY, JSON.stringify(trimmed));
  }catch(e){ /* stockage indisponible : l'assistant reste fonctionnel, sans historique persistant */ }
}

/**
 * Bloque le scroll de la page derrière le panneau de chat. `overflow:hidden`
 * seul ne suffit pas sur iOS Safari (le body continue de "rebondir" au
 * doigt) — on fige le body en position:fixed à son offset de scroll actuel,
 * technique standard cross-navigateur, puis on restaure l'offset exact à
 * la fermeture (vlnUnlockPageScroll).
 */
let vlnLockedScrollY = 0;
function vlnLockPageScroll(){
  vlnLockedScrollY = window.scrollY || document.documentElement.scrollTop || 0;
  const body = document.body;
  body.style.position = 'fixed';
  body.style.top = -vlnLockedScrollY + 'px';
  body.style.left = '0';
  body.style.right = '0';
}
function vlnUnlockPageScroll(){
  ...
  body.style.left = '';
  body.style.right = '';
  window.scrollTo(0, vlnLockedScrollY);
}
